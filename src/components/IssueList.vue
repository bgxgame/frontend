...
<template>
  <div class="space-y-6">
    ...
    <div v-else-if="issues.length === 0" class="text-center py-32 bg-white rounded-xl border border-dashed border-[#E6E7E8]">
      <div class="w-12 h-12 bg-[#FAFAFC] rounded-full flex items-center justify-center mx-auto mb-4 border border-[#F0F0F2]">
        <svg class="w-6 h-6 text-[#D1D1D6]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
      </div>
...
// 使用显式类型定义 reactive
const kanbanColumns = reactive<KanbanColumn[]>([
  { id: 'todo', label: '待办 (Todo)', items: [] },
  { id: 'in_progress', label: '进行中 (Started)', items: [] },
  { id: 'done', label: '已完成 (Done)', items: [] }
]);

// 1. 获取任务数据：支持静默更新，不显示全屏 loading
const fetchIssues = async (silent = false) => {
  ...
  try {
    ...
    // TypeScript 修复点：使用 find 安全定位列，避免索引访问的 undefined 风险
    const todoCol = kanbanColumns.find(c => c.id === 'todo');
    const inProgressCol = kanbanColumns.find(c => c.id === 'in_progress');
    const doneCol = kanbanColumns.find(c => c.id === 'done');

    if (todoCol) {
      todoCol.items = issues.value.filter(i => i.status === 'todo' || i.status === 'backlog');
    }
    if (inProgressCol) {
      inProgressCol.items = issues.value.filter(i => i.status === 'in_progress');
    }
    if (doneCol) {
      doneCol.items = issues.value.filter(i => i.status === 'done');
    }
  } catch (err) {
...
const formatStatus = (s: string) => {
  const map: Record<string, string> = {
    'todo': 'Todo', 'in_progress': 'Started', 'done': 'Done', 
